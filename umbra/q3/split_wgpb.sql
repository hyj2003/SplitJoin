...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 160) AS intermediate_4
     ON forceorder(intermediate_4.obj = intermediate_6.sub)) AS intermediate_9 JOIN 
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 302) AS intermediate_0
     ON forceorder(intermediate_0.sub = intermediate_2.sub)) AS intermediate_8 ON forceorder(intermediate_8.intermediate_0_obj = intermediate_9.intermediate_4_sub AND intermediate_8.intermediate_2_obj = intermediate_9.intermediate_6_obj)) AS intermediate_10
        ;
SELECT COUNT(*)
FROM 
(SELECT intermediate_11.intermediate_5_sub AS intermediate_11_intermediate_5_sub,
...
FROM 
    ...
    FROM wgpb t
    LEFT JOIN sub_obj c ON t.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 160) AS intermediate_14
