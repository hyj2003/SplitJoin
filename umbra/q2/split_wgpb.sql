...
CREATE TEMP TABLE group_obj_f AS (
    SELECT obj AS val, COUNT(*) AS cnt 
    FROM (SELECT sub, obj FROM wgpb WHERE
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_0
     JOIN 
    (SELECT t2.* 
    FROM wgpb AS t2
    LEFT JOIN sub_obj_0 c ON t2.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_2
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_4
     JOIN 
    (SELECT t3.* 
    FROM wgpb AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_6
     ON forceorder(intermediate_6.sub = intermediate_4.obj)) AS intermediate_9 ON forceorder(intermediate_8.intermediate_2_obj = intermediate_9.intermediate_6_obj AND intermediate_8.intermediate_0_sub = intermediate_9.intermediate_4_sub)) AS intermediate_10
        ;
SELECT COUNT(*)
FROM 
...
FROM 
    ...
    LEFT JOIN sub_obj_0 c ON t1.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_0
     ON forceorder(intermediate_0.sub = intermediate_5.sub)) AS intermediate_12 JOIN 
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_2 s
        WHERE s.cnt > 134) c
        ON t3.sub = c.val) AS intermediate_7
     JOIN 
    (SELECT t2.* 
    FROM wgpb AS t2
    LEFT JOIN sub_obj_0 c ON t2.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_2
     ON forceorder(intermediate_2.obj = intermediate_7.obj)) AS intermediate_11 ON forceorder(intermediate_11.intermediate_2_sub = intermediate_12.intermediate_0_obj AND intermediate_11.intermediate_7_sub = intermediate_12.intermediate_5_obj)) AS intermediate_13
        ;
SELECT COUNT(*)
FROM 
(SELECT intermediate_18.intermediate_3_sub AS intermediate_18_intermediate_3_sub,
       intermediate_18.intermediate_16_sub AS intermediate_18_intermediate_16_sub,
...
FROM 
    ...
    FROM wgpb AS t4
    LEFT JOIN sub_obj_2 c ON t4.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_14
     ON forceorder(intermediate_1.sub = intermediate_14.sub)) AS intermediate_19 JOIN 
...
FROM 
    ...
    FROM wgpb AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 134) AS intermediate_16
     ON forceorder(intermediate_3.obj = intermediate_16.obj)) AS intermediate_18 ON forceorder(intermediate_18.intermediate_3_sub = intermediate_19.intermediate_1_obj AND intermediate_18.intermediate_16_sub = intermediate_19.intermediate_14_obj)) AS intermediate_20
        ;
SELECT COUNT(*)
FROM 
(SELECT intermediate_21.intermediate_3_sub AS intermediate_21_intermediate_3_sub,
...
FROM 
    ...
    JOIN (
     ...
     ON forceorder(intermediate_1.sub = intermediate_15.sub)) AS intermediate_22 JOIN 
(SELECT intermediate_3.sub AS intermediate_3_sub,
       intermediate_3.obj AS intermediate_3_obj,
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_2 s
        WHERE s.cnt > 134) c
        ON t3.sub = c.val) AS intermediate_17
     JOIN 
    (SELECT t2.* 
    FROM wgpb AS t2
    JOIN (
        ...
        WHERE s.cnt > 134) c
        ON t2.sub = c.val) AS intermediate_3
     ON forceorder(intermediate_3.obj = intermediate_17.obj)) AS intermediate_21 ON forceorder(intermediate_21.intermediate_3_sub = intermediate_22.intermediate_1_obj AND intermediate_21.intermediate_17_sub = intermediate_22.intermediate_15_obj)) AS intermediate_23
        ;