...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_0
     JOIN 
    (SELECT t2.* 
    FROM orkut AS t2
    LEFT JOIN sub_obj_0 c ON t2.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_2
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_6
     JOIN 
    (SELECT t3.* 
    FROM orkut AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_4
...
FROM 
    ...
    FROM orkut AS t4
    LEFT JOIN sub_obj_2 c ON t4.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_16
...
FROM 
    ...
    FROM orkut AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 448) AS intermediate_14
