...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_6
     JOIN 
    (SELECT t3.* 
    FROM gplus AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_4
...
FROM 
    ...
    FROM gplus AS t4
    LEFT JOIN sub_obj_2 c ON t4.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_16
...
FROM 
    ...
    FROM gplus AS t3
    LEFT JOIN sub_obj_2 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_14
