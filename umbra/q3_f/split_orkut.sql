...
CREATE TEMP TABLE group_obj_f AS (
    SELECT obj AS val, COUNT(*) AS cnt 
    FROM (SELECT sub, obj FROM orkut WHERE sub % 3 = 1) AS t1 
    GROUP BY obj 
    ORDER BY cnt DESC 
    LIMIT 5000
);
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 597) AS intermediate_0
     JOIN 
    (SELECT t2.* 
    FROM orkut AS t2
    LEFT JOIN sub_obj_0 c ON t2.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 597) AS intermediate_2
...
FROM 
    ...
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 414) AS intermediate_6
     JOIN 
    (SELECT t3.* 
    FROM orkut AS t3
    LEFT JOIN sub_obj_2 c ON t3.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 414) AS intermediate_4
...
FROM 
    ...
    LEFT JOIN sub_obj_0 c ON t2.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 597) AS intermediate_2
     ON forceorder(intermediate_2.obj = intermediate_7.obj)) AS intermediate_11 ON forceorder(intermediate_11.intermediate_2_sub = intermediate_12.intermediate_0_sub AND intermediate_11.intermediate_7_sub = intermediate_12.intermediate_5_obj)) AS intermediate_13
        ;EXPLAIN ANALYZE 
SELECT COUNT(*)
FROM 
(SELECT intermediate_18.intermediate_1_obj AS intermediate_18_intermediate_1_obj,
...
FROM 
    ...
    FROM orkut AS t3
    LEFT JOIN sub_obj_2 c ON t3.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 414) AS intermediate_14
...
FROM 
    ...
    FROM orkut AS t4
    LEFT JOIN sub_obj_2 c ON t4.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 414) AS intermediate_16
