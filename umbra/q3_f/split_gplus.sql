...
CREATE TEMP TABLE group_obj_f AS (
    SELECT obj AS val, COUNT(*) AS cnt 
    FROM (SELECT sub, obj FROM gplus WHERE
...
FROM 
    ...
    FROM (SELECT sub, obj FROM gplus WHERE
    ascii(substring(md5(sub), 1, 1)) % 3 = 1) AS t1
    LEFT JOIN sub_obj_1 c ON t1.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_2
     JOIN 
    (SELECT t3.* 
    FROM gplus AS t3
    LEFT JOIN sub_obj_1 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_0
     ON forceorder(intermediate_2.obj = intermediate_0.sub)) AS intermediate_8 JOIN 
...
FROM 
    (SELECT t4.* 
    FROM gplus AS t4
    LEFT JOIN sub_obj_3 c ON t4.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 684) AS intermediate_4
     JOIN 
    (SELECT t2.* 
    FROM gplus AS t2
    LEFT JOIN sub_obj_3 c ON t2.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 684) AS intermediate_6
     ON forceorder(intermediate_6.obj = intermediate_4.obj)) AS intermediate_9 ON forceorder(intermediate_8.intermediate_2_sub = intermediate_9.intermediate_6_sub AND intermediate_8.intermediate_0_obj = intermediate_9.intermediate_4_sub)) AS intermediate_10
        ;EXPLAIN ANALYZE 
SELECT COUNT(*)
FROM 
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_3 s
        WHERE s.cnt > 684) c
        ON t2.obj = c.val) AS intermediate_7
     JOIN 
    (SELECT t1.* 
    FROM (SELECT sub, obj FROM gplus WHERE
    ascii(substring(md5(sub), 1, 1)) % 3 = 1) AS t1
    LEFT JOIN sub_obj_1 c ON t1.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_2
     ON forceorder(intermediate_2.sub = intermediate_7.sub)) AS intermediate_11 JOIN 
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_3 s
        WHERE s.cnt > 684) c
        ON t4.obj = c.val) AS intermediate_5
     JOIN 
    (SELECT t3.* 
    FROM gplus AS t3
    LEFT JOIN sub_obj_1 c ON t3.sub = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 585) AS intermediate_0
     ON forceorder(intermediate_0.obj = intermediate_5.sub)) AS intermediate_12 ON forceorder(intermediate_11.intermediate_2_obj = intermediate_12.intermediate_0_sub AND intermediate_11.intermediate_7_obj = intermediate_12.intermediate_5_obj)) AS intermediate_13
        ;EXPLAIN ANALYZE 
SELECT COUNT(*)
FROM 
(SELECT intermediate_18.intermediate_16_sub AS intermediate_18_intermediate_16_sub,
       ...
       intermediate_18.intermediate_16_obj AS intermediate_18_intermediate_16_obj,
       intermediate_19.intermediate_1_obj AS intermediate_19_intermediate_1_obj,
       intermediate_19.intermediate_14_obj AS intermediate_19_intermediate_14_obj,
       intermediate_19.intermediate_1_sub AS intermediate_19_intermediate_1_sub,
       intermediate_19.intermediate_14_sub AS intermediate_19_intermediate_14_sub
FROM 
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_1 s
        WHERE s.cnt > 585) c
        ON t1.obj = c.val) AS intermediate_3
     JOIN 
    (SELECT t2.* 
    FROM gplus AS t2
    LEFT JOIN sub_obj_3 c ON t2.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 684) AS intermediate_16
     ON forceorder(intermediate_3.sub = intermediate_16.sub)) AS intermediate_18 JOIN 
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_1 s
        WHERE s.cnt > 585) c
        ON t3.sub = c.val) AS intermediate_1
     JOIN 
    (SELECT t4.* 
    FROM gplus AS t4
    LEFT JOIN sub_obj_3 c ON t4.obj = c.val
    WHERE CASE WHEN c.cnt IS NULL THEN 0 ELSE c.cnt END <= 684) AS intermediate_14
     ON forceorder(intermediate_1.obj = intermediate_14.sub)) AS intermediate_19 ON forceorder(intermediate_18.intermediate_3_obj = intermediate_19.intermediate_1_sub AND intermediate_18.intermediate_16_obj = intermediate_19.intermediate_14_obj)) AS intermediate_20
        ;EXPLAIN ANALYZE 
SELECT COUNT(*)
FROM 
(SELECT intermediate_21.intermediate_3_obj AS intermediate_21_intermediate_3_obj,
       ...
       intermediate_22.intermediate_15_obj AS intermediate_22_intermediate_15_obj,
       intermediate_22.intermediate_1_sub AS intermediate_22_intermediate_1_sub,
       intermediate_22.intermediate_15_sub AS intermediate_22_intermediate_15_sub
FROM 
(SELECT intermediate_3.obj AS intermediate_3_obj,
       intermediate_3.sub AS intermediate_3_sub,
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_3 s
        WHERE s.cnt > 684) c
        ON t2.obj = c.val) AS intermediate_17
     ON forceorder(intermediate_3.sub = intermediate_17.sub)) AS intermediate_21 JOIN 
(SELECT intermediate_1.obj AS intermediate_1_obj,
       intermediate_1.sub AS intermediate_1_sub,
...
FROM 
    ...
    JOIN (
        ...
        FROM sub_obj_3 s
        WHERE s.cnt > 684) c
        ON t4.obj = c.val) AS intermediate_15
     JOIN 
    (SELECT t3.* 
    FROM gplus AS t3
    JOIN (
        ...
        WHERE s.cnt > 585) c
        ON t3.sub = c.val) AS intermediate_1
     ON forceorder(intermediate_1.obj = intermediate_15.sub)) AS intermediate_22 ON forceorder(intermediate_21.intermediate_3_obj = intermediate_22.intermediate_1_sub AND intermediate_21.intermediate_17_obj = intermediate_22.intermediate_15_obj)) AS intermediate_23
        ;