...
profiling_path = "duckdb_split_profile/profile.log"
time_info = {}
...
def exe_final_join(con):
    ...
    if min_time is not None:
        total_time += min_time
        with open(f"./duckdb_split_profile/{query_name}/{db_name}.log", "w") as f:
            for i, sql in enumerate(final_joins):
                if i == 0:
                    union_sql = sql
                else:
...
def join_phase(con, joins: List[Join], light: np.ndarray):
    ...
    for i, j in enumerate(joins):
        ...
        # print(basetable_dict[j.table1], basetable_dict[j.table2], j.factor, j.factor_L, light[i])
...
if __name__ == "__main__":
    ...
    db_names = ['wgpb', 'orkut', 'uspatent', 'skitter', 'topcats']
    # db_names = ['wgpb', 'topcats']
